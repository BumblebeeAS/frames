...
[[nodiscard]] constexpr auto to_ypr_deg(const Rotation<FrameConvention1, FrameConvention2>& rotation) -> YPR_Deg {
    ...
    const auto sinr_cosp = (rotation.getW() * rotation.getX() + rotation.getY() * rotation.getZ());
    const auto cosr_cosp = 0.5 - (rotation.getX() * rotation.getX() + rotation.getY() * rotation.getY());
    const auto roll = math::atan2(sinr_cosp, cosr_cosp);

    // pitch (y-axis rotation)
    const auto sinp = 2 * (rotation.getX() * rotation.getZ() - rotation.getW() * rotation.getY());
    const auto pitch = math::is_close(math::abs(sinp), 1.0)
                           ? (sinp > 0 ? -math::detail::pi<double>() / 2 : math::detail::pi<double>() / 2)
                           : math::asin(-sinp);

    // yaw (y-axis rotation)
    const auto siny_cosp = (rotation.getW() * rotation.getZ() + rotation.getX() * rotation.getY());
    const auto cosy_cosp = 0.5 - (rotation.getY() * rotation.getY() + rotation.getZ() * rotation.getZ());
    const auto yaw = math::atan2(siny_cosp, cosy_cosp);
    return {math::rad2deg(yaw), math::rad2deg(pitch), math::rad2deg(roll)};
}
